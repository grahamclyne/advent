 ...
 #PART 2
    for cmd in input:
        ...
        if('swap' in cmd and 'position' in cmd):
            ...
            goal = 0
            #check each possibility, if it lines up, get difference and rotate there
            for i in range(8):
                start = 0
                if(i >= 4):
                    start = 2 + i + i
                ...
                if(start == index):
                    goal = i
                    break
            start_list = rotate2(start_list, (goal-index)+8)
            
        elif('rotate' in cmd):
            dir = 'left'
            if(cmd[1] == 'left'):
                dir = 'right'
            start_list = rotate(start_list, int(cmd[2]), dir)

        elif('move' in cmd):
            temp = start_list[int(cmd[5])]
            start_list.remove(start_list[int(cmd[5])])
            start_list.insert(int(cmd[2]), temp)
...
#ghcdfbea,eahgdcfb,dgfaehbc not the right answer for part 2